...
const RecommendContextProvider = ({ children }) => {
  ...
  const getRec = async () => {
    try {
      ...
      let res = await axios(`http://34.134.203.27/recommendations/`, config);
      dispatch({
        type: "GET_RECOMMEND",
        payload: res.data,
      });
  ...
  const supportBot = async () => {
    try {
      ...
      let res = await axios(
        `${API}support-bot/${window.location.search}`,
        config
      );
      dispatch({
        type: "GET_QUATIONS",
  ...
  const supportBotAnswer = async (id) => {
    try {
      ...
      let res = await axios(
        `http://34.134.203.27/support-bot/?q=${id}`,
        config
      );
      dispatch({
        type: "GET_ANSWER",
  ...
  const search = async () => {
    try {
      ...
      let { data } = await axios(
        `${API}search/${window.location.search}`,
        config
      );
      dispatch({
        type: "GET_SEARCH",
  ...
  return (
    <recommendContext.Provider value={values}>
      {children}
